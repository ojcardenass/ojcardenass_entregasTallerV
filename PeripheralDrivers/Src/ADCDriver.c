...
void adcExternalConfig(ADC_Config_t *adcConfig){
	//Activamos el evento externo, segun el tipo de flanco
	if(adcConfig->flank |= EXTERNAL_INTERRUPT_FALLING_EDGE){
	...
	//Evento externo seleccionado para lanzar la conversión ADC (evento es pin 1)
	ADC1->CR2 |= (adcConfig->eventExt<<ADC_CR2_EXTSEL_Pos);
...
void MultiCanalConfig(ADC_Config_t *adcConfig, uint8_t numCan){
	...
	/* 8. Configuramos la secuencia y cuantos elementos hay en la secuencia */
	ADC1 -> SQR1 |= (numCan - 1) << ADC_SQR1_L_Pos;
